...
Component.entryPoint = function(NS){
    ...
    ProfileWidget.prototype = {
        ...
        onClick: function(el){
            if (this.pages['avatar'].onClick(el)){
                return true;
            }
            ;
            if (this.pages['password'].onClick(el)){
                return true;
            }
            ;
            var TId = this._TId;
            if (el.id == TId['widget']['bsave']){
                this.pages['editor'].save();
                return true;
            }
            return false;
        },
    ...
    PasswordEditorWidget.prototype = {
        elv: function(name){
            return Brick.util.Form.getValue(this._TM.getEl('password.' + name));
        },
        init: function(container, userid){
            userid = userid || Brick.env.user.id;
            this.userid = userid;
